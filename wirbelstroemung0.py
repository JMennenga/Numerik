...
class Wirbelstroemung:
    ...
    def setup(self):
        ...
        [self.b_rand, self.dir_rand] = [a.reshape(self.gridlength) for a in [
            self.b_rand, self.dir_rand]]
        ...
        self.Lap_rand = (1/self.h**2) * (sparse.diags(self.dir_rand == 0x1, dtype=bool)*b1[0]  # S-Rand
                                         + sparse.diags(self.dir_rand == 0x2,
                                                        dtype=bool)*b1[2]  # N-Rand
                                         + sparse.diags(self.dir_rand == 0x4,
                                                        dtype=bool)*b1[1]  # O-Rand
                                         + sparse.diags(self.dir_rand == 0x8, dtype=bool) * \
                                         b1[3]  # W-Rand  (heh SNOW...)

                                         + sparse.diags(self.dir_rand == 0x5,
                                                        dtype=bool)*b2[0]  # SO
                                         + sparse.diags(self.dir_rand == 0x6,
                                                        dtype=bool)*b2[1]  # NO
                                         + sparse.diags(self.dir_rand == 0x9,
                                                        dtype=bool)*b2[3]  # SW
                                         + sparse.diags(self.dir_rand == 0xA,
                                                        dtype=bool)*b2[2]  # SO
                                         )
    ...
    def rk4(self, stopevent, f, ww):
        ...
        while not stopevent.is_set():
            [k1, u, v, draw_ww, spaltensum] = f(ww, t)
            ...
            yield [t, draw_ww, u, v, spaltensum]
    ...
    def rhs(self, w, t):
        ...
        self.ww = w + ww_rand
        f_matrix = -((sparse.diags(ax, dtype=bool) * self.D1w * (sparse.diags(u)))
                     + (sparse.diags(np.logical_not(ax), dtype=bool) * self.D1o * (sparse.diags(u)))
                     + (sparse.diags(ay, dtype=bool) * self.D1n * (sparse.diags(v)))
                     + (sparse.diags(np.logical_not(ay), dtype=bool) * self.D1s * (sparse.diags(v)))

                     # - self.kin_vis * (self.Lap0)  # Reibungsterm
                     )
        ...
        return [rhs, u, v, self.ww, spaltensum]
