...
sim_options['image'] = np.array(plt.imread(filedialog.askopenfilename()))
...
simobj = wirbelstroemung0.Wirbelstroemung(sim_options)
simobj.setup()
w0 = simobj.get_w0()
...
plot.set_title('Strömungssimulation', fontsize=20, fontweight=0,
               color='C0', loc='left', style='italic')
...
#                         vmax=np.max(w0))
norm = colors.SymLogNorm(0.000001,  vmin=np.min(w0),
                         vmax=np.max(w0))
...
Q = plot.quiver(QXpos, QYpos,
                np.ones(sim_options['shape'])[Qmesh],
                np.ones(sim_options['shape'])[Qmesh])
...
def animation(frame):
    ...
    ww_n = spaltensum[np.logical_not(simobj.b_rand)]
    ww = spaltensum.reshape(sim_options['shape'])
    im.set_data(ww)

    # norm = colors.Normalize(vmin=np.min(ww_n),
    #                         vmax=np.max(ww_n))
    norm = colors.SymLogNorm(0.000001,  vmin=np.min(ww_n),
                             vmax=np.max(ww_n))
